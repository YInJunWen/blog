...
```js
let obj = { name: 'pear' };
let obj2 = { color: 'yellow' };
function egg() {}
Object.setPrototypeOf(egg.prototype, obj); // 语句整体返回target对象
console.log(new egg().name); // 'pear'

Reflect.setPrototypeOf(egg.prototype, obj2); // 语句整体返回 true
console.log(new egg().color); // 'yellow'
```
